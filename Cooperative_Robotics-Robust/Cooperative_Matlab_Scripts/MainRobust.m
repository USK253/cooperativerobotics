...
for t = 0:deltat:end_time
    ...
    %% Ex 1.1.2: "Safe Navigation aciton": Vehicle Position control task (Action-Defining) 
    ...
    %% Ex 2.1:  "Landing action" : Altitude control task (Action Defining) 
    ...
    %% Ex 3: "Allignment x_vehicle/rock action" : allignment control task (Operetional Prerequisite) 
    [Qp, ydotbar] = iCAT_task(uvms.A.xi,     uvms.Jxi,    Qp, ydotbar, uvms.xdot.xi,  0.0001,   0.01, 10);  
    

    %% Ex 4.1: Vehicle Null velocity non-reactive control task (Optimization) 
    [Qp, ydotbar] = iCAT_task(uvms.A.null,    uvms.Jnull,    Qp, ydotbar, uvms.xdot.null,  0.0001,   0.01, 10);

    %% Ex 0 : tool position control task (Action Defining) 
